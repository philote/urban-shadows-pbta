...
Hooks.once("init", () => {
  ...
  CONFIG.Actor.documentClass = UrbanShadowsActorMixin(
    game.pbta.documents.ActorPbta
  );
  ...
  CONFIG.Actor.typeLabels["urban-shadows-pbta.city-hub"] =
    "TYPES.Actor.city-hub";
  ...
  if (game.settings.get("urban-shadows-pbta", "firstTime")) {
    ...
    const callback = async () => {
      ...
      const worldData = {
        id: game.world.id,
        action: "editWorld",
        background: `modules/urban-shadows-pbta/assets/magpie/cover.webp`,
      };
      let response;
      try {
    ...
    foundry.applications.api.DialogV2.confirm({
      window: {
        title: game.i18n.localize("US2E.Settings.startup.dialog.title"),
      },
